...
public class AS {
  ...
  Set<Integer> customers() {return customers;};
  Set<Integer> peers() {return peers;};
  Set<Integer> providers() {return providers;};
  Set<Integer> neighbours() {return neighbours;};
  int number() { return number;}

  void AddCustomer(int as_number) {
    if (isNeighbour(as_number)) { return; }
  ...
  boolean malicious()  { return state == AS_STATE.AS_MALICIOUS; }
  boolean legacy()  { return state == AS_STATE.AS_LEGACY; }
  boolean adopter()  { return state == AS_STATE.AS_ADOPTER; }

  void set_optattr_processing(AS_DISCARD_OPTATTR discard) { optattr_processing_ = discard; }
  boolean optattr_discard_attr()  { return optattr_processing_ == AS_DISCARD_OPTATTR.DISCARD_OPTATTR; }
  boolean optattr_discard_prefix()  { return optattr_processing_ == AS_DISCARD_OPTATTR.DISCARD_PREFIX; }

  RIR region()  { return region; }
  ...
  static boolean is_geographical_region(RIR region) {
    return (region == RIR.AFRINIC) || (region == RIR.APNIC) || (region == RIR.ARIN)
        || (region == RIR.LACNIC) || (region == RIR.RIPE_NCC);
  }
  ...
  static boolean is_categories_region(RIR region){
    return (region == RIR.LARGE_ISPS) || (region == RIR.MEDIUM_ISPS) ||
        (region == RIR.SMALL_ISPS) || (region == RIR.STUBS);
  }

  static String region_to_txt(RIR region){
    if (region == RIR.AFRINIC) { return "Africa"; }
    else if (region == RIR.APNIC) { return "Asia"; }
    else if (region == RIR.ARIN) { return "North America"; }
    else if (region == RIR.LACNIC) { return "South America"; }
    else if (region == RIR.RIPE_NCC) { return "Europe"; }
    else if (region == RIR.ALL) { return "All"; }
    else if (region == RIR.LARGE_ISPS) { return "Large ISPs"; }
    else if (region == RIR.MEDIUM_ISPS) { return "Medium ISPs"; }
    else if (region == RIR.SMALL_ISPS) { return "Small ISPs"; }
    else if (region == RIR.STUBS) { return "Stubs"; }
    else if (region == RIR.BIGGEST_CPS) { return "BIGGEST_CPS"; }
    else { return "other"; }
  }
