...
public class GraphProcessor {
  ...
  GraphProcessor(BGPGraph graph, RIR attackers_region, RIR victims_region, boolean filter_two_neighbours){
    this.graph_ = graph;
  ...
  Map<Integer, RoutingTable> Dijekstra(int dest_as_number, int attacker_as_number, int hops, boolean filter_by_length, boolean first_legacy, boolean skip_attacker){
    ...
    route_tables.put(dest_as_number, new RoutingTable(dest_as_number, graph_, filter_by_length, filter_two_neighbours_, true));
    ...
    //   route_tables.put(attacker_as_number, new RoutingTable(attacker_as_number, graph_, filter_by_length, filter_two_neighbours_));
    //   List<Integer> spoofed_route = get_spoofed_route_by_hops(dest_as_number, attacker_as_number, hops, first_legacy);
    ...
    while (!q.isEmpty()) {
      ...
      // iterate customers
      Route optional_route = route_tables.get(currAS.number()).get_route_or_null(dest_as_number, ADVERTISEMENT_DEST.ADVERTISE_TO_CUSTOMER);
      boolean prot_prev;
      if (optional_route != null) {
        ...
        for (Integer customer : currAS.customers()) {
          ...
          if (!route_tables.containsKey(customer)) {
            route_tables.put(customer, new RoutingTable(customer, graph_, filter_by_length, filter_two_neighbours_));
          }
      ...
      // iterate peers
      optional_route = route_tables.get(currAS.number()).get_route_or_null(dest_as_number, ADVERTISEMENT_DEST.ADVERTISE_TO_PEER);
      ...
      if (optional_route != null) {
        ...
        for (Integer peer : currAS.peers()) {
          ...
          if (!route_tables.containsKey(peer)) {
            route_tables.put(peer, new RoutingTable(peer, graph_, filter_by_length, filter_two_neighbours_));
          }
      ...
      // iterate providers
      optional_route =route_tables.get(currAS.number()).get_route_or_null(dest_as_number, ADVERTISEMENT_DEST.ADVERTISE_TO_PROVIDER);
      ...
      if (optional_route != null) {
        ...
        for (Integer provider : currAS.providers()) {
          ...
          if (!route_tables.containsKey(provider)) {
            route_tables.put(provider, new RoutingTable(provider, graph_, filter_by_length, filter_two_neighbours_));
          }
  ...
  //Map<Integer, shared_ptr<RoutingTable> >* Dijekstra_avichai(int dst_as_number, int attacker_as_number, int hops, boolean filter_by_length) const;
  double analyze_attacker_success(Map<Integer, RoutingTable > attacker_results, int dst_as_number, int filtering_mode)  {
    return 1;
  }
